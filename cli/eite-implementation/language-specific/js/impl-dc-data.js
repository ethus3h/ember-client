...
async function implStrDcDataLookupByValue(strDataset, intFilterField, strFilterValue, intDesiredField) {
    ...
    for (let intRow = 1; intRow < intLength; intRow++) {
        if(dcData[strDataset][intRow].data[0][intFilterField] === strFilterValue) {
            strReturn = dcData[strDataset][intRow].data[0][intDesiredField]; await assertIsStr(strReturn); console.log("Abrube", strReturn); return strReturn;
        }
    }
    await assertIsStr(strReturn); console.log("ABoruhre", strReturn); return strReturn;
}
