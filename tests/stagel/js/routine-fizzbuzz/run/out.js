async function fbSwitcher(intNum) {
    ...
    if (await implEq(0, await implMod(intNum, 3)/* Could also write it like this, with the condition on another line */
    {
        /* Comment 4 */
...
await fizzBuzz()/* A string is an array of 8-bit bytes. */
{
    /* return-assert -n # the output should be non-empty. */
    let intCounter = 1while (!(await implEq(intCounter, 100))) {
        intCounter = await implAdd(intCounter, 1)let strTemp = ''strTemp = await fbSwitcher(intCounter)
